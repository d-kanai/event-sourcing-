...
describe('GetAccountQuery', () => {
  ...
  describe('execute', () => {
    it('IDで既存のアカウントを取得できる', async () => {
      const created = await createAccountCommand.execute({ userId: "test-user", initialBalance: 500 });
    ...
    it('作成後のアカウントを正しい残高で取得できる', async () => {
      ...
      const created = await createAccountCommand.execute({ userId: "test-user", initialBalance });
    ...
    it('複数の異なるアカウントを正しく取得できる', async () => {
      const account1 = await createAccountCommand.execute({ userId: "test-user", initialBalance: 100 });
      const account2 = await createAccountCommand.execute({ userId: "test-user", initialBalance: 200 });
